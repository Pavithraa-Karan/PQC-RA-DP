...
import dp_classify
import pandas as pd # <-- ADDED PD IMPORT HERE FOR SCOPE FIX (from previous turn)
...
if mode == "Single asset demo":
    ...
    if uploaded:
        ...
        if not out_csv:
            ...
            # Removed redundant pd import here (moved to top of file)
    ...
    if uploaded:
        ...
        if X_num.empty and X_cat.empty:
            ...
            # Must provide all classes (0, 1, 2, 3) on the first fit/partial_fit call
            ...
            st.markdown("Provide a single-row CSV (same columns as scored CSV) or paste a CSV row. You must supply the true label for the new instance (0=Low, 1=Medium, 2=High, 3=Critical).")
            ...
            if new_in.strip() == "":
                ...
                try:
                    ...
                    # This now correctly uses the imported pd
            ...
            if new_row is not None:
                ...
                # Derive label suggestion (if QARS category is in the new row)
                try:
                    suggested_label_array = dp_classify.derive_label(new_row, label_col="QARS_category")
                    suggested = int(suggested_label_array[0]) if suggested_label_array.size > 0 else 1
                except ValueError:
                    # Default to 1 (Medium) if QARS_category is missing in the single row
                    suggested = 1
                    
                true_label = st.selectbox("True label for new instance (0=Low, 1=Medium, 2=High, 3=Critical)", options=[0,1,2,3], index=suggested)
                ...
                try:
                    ...
                    # FIX: Change classes to reflect the 4 multi-class categories (0, 1, 2, 3)
                    clf.partial_fit(Xn_trans, np.array([true_label]), classes=np.array([0, 1, 2, 3]))
